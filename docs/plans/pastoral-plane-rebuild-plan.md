...
> **结构修订（2026-09-20 夜图南第二次拍板，v3 单主线）**：**取消全部分支与多结局**——三个抉择点/27 组合/R1-R5 结局规则表/D5 轻分支/D7 结局对称全部作废；改为**三个「关键供书节点」**：每幕危机=位面亟需的知识，玩家把对应书**抄完+借出**即供给成功（软门：未供上剧情原地等待，永不卡死）；剧情冲突保留、由知识收场；损失模型**留病倒删离别**（静默邮箱保留，余烬离别/流浪熟客取消）。契约层待终审按白话版全面修订。
