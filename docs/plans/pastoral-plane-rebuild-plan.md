...
> **立意修订（2026-09-20 晚图南拍板，优先级高于 v2.1 本文的叙事表述）**：开场不是「收到求救信」，而是**位面之门开启后图书馆显形在 w 位面中，有缘人可见可入**；小艾拉是该位面第一个访客（推门借书、留信缮写室）。世界规则四条：①只有图书馆的书可带出馆门；②非有缘人看不见图书馆与书；③知识靠访客自行学习/抄录传播（誊抄=造书、借书=传播的驱动链）；④**不同位面的访客互相看不见、不能交流**（留坑：未来「图书馆位面庆典」全员欢聚+小剧场，进 backlog）。形态=建筑降临+馆长留守（幕1-2 玩家不出馆，纯书信往来；幕3 谷门洞开=馆长首次亲眼见山谷）。
> **通俗白话版（按新立意重写，叙事侧权威）：`pastoral-plane-plain.md`（2026-09-20 应图南要求而作——零术语讲故事：五幕剧情/五个角色/三个抉择/三结局/机制全白话，完全没读过方案的人也能懂，从这个读起）**
