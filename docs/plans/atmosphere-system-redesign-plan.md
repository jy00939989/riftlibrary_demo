...
state.library.freeAtmosphere   // 自由氛围池（软顶 3000 主闸，见 §五）
...
| 借阅区 Lv3-7 氛围成本 | **0 / 80 / 300 / 700 / 1500**（D14 成本后置） | 产能杠杆最贵且前置 = 前期死锁（评审 P0-11）；低等级压到零，成本集中到高等级；新列命名 `upgradeAtmoCost` |
| 修复室 Lv2-5 氛围成本 | 60 / 150 / 350 / 700 | 定价函数在 capacity.js |
| 设施一次性加成 | 等级×40~60 氛围 | **新升级走新公式；老档补发按历史实发（借阅区/缮写室 15/级，修复室 0——它从不发氛围，评审 P2-10）** |
| 设施每日产出 | **修缮材料 [PLACEHOLDER] 个/级/日**（D10，不产氛围） | 修缮材料 = Phase 3 任务链物资；离线补算上限 24h |
| 升阶仪式消耗 | 1→2: 30 / 2→3: 100 / 3→4: 220 / 4→5: 484（×2.2 阶梯） | peak 达阈值才可发起 |
| 单书借阅磨损 | **乘性（D13）：×(1 + 0.6 × wearSteps/15)，满磨损 ×1.6** | 原加性方案在 Lv7 时磨损占比 83% 稀释升级价值（评审 P1-7）；重抄清零；注入 `getDamageChance` 第 3 参（默认 0 向后兼容） |
| 典藏版还书加成 | 智慧之光 ×2，氛围 ×3 | 与典藏诊断收藏定价匹配 |
| 自由池兑灵感 | 50 : 1，**每日兑换上限 200 池（=4 灵感）** | 防灵感通胀；软顶见 §五 |
| 自由池软顶 | **3000（主闸，D12）**，超出停增或 10:1 折损 [PLACEHOLDER] | 与藏书厅 500 顶同构，最简单可控 |
| 火灾氛围罚金 | 15，**可透支自由池（D9）** | 先扣库存不足扣自由池；零库存不再免疫火灾 |
...
0. **Phase 0 收支测算脚本**：新模型氛围/灵感日收支仿真（来源收窄 + 还书主导产能 vs 仪式/升级/维持费/兑换），**用真实存档数据标定还书周转率**（评审 3.2 手估 1.5 次/日/位是猜测值）；磨损速率↔重抄频率↔灵感消耗链条校核；全部 [PLACEHOLDER] 定值——**不过测算不开工**
