...
2. **商店刷新**：采用**动态轻偏置**（拥有卷数越多，缺失卷权重越高：1/6→2.5x，5/6→5.0x），配合**临门一脚保底**（gap === 1 时每轮刷新保 1 条缺失卷）。见 5.1 / 7.1。
...
9. **访客吐槽**：**二阶段再做**。本次预留 `data/volume_quips.js` 和 `js/volumeQuips.js` 接口但不接入 `js/visitors.js`。见 10.8。
10. **保底机制**：已加入临门一脚保底。见 5.1 / 7.1。
11. **修缮箱扩容**：初始 **3 格免费**，最多 **20 格**，价格曲线见 11.3。
12. **裴舟荐书**：优先推荐缺失单卷，无缺失时回退到普通书逻辑。见 7.5。
13. **新增成就**：「卷轴收藏家」——首次合成一本典藏版。见 4.7。
