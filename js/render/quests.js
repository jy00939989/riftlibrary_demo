...
export function renderCharacterCard(planeId, charId) {
  ...
  header.innerHTML = `
    <button class="char-back-btn px-3 py-2 bg-wood/10 hover:bg-wood/20 rounded-lg text-sm font-bold transition-all">${t('backToPlane')}</button>
    <span class="text-4xl">${char.emoji}</span>
    <div>
      <h2 class="font-display text-xl font-bold">${char.name}</h2>
      <p class="text-xs text-ink-light">${t('characterHeaderInfo').replace('{role}', char.role).replace('{favor}', cd.favor || 0)}</p>
    </div>
  `;
  ...
  intro.innerHTML = `
    ...
    ${!cd.met ? `<p class="text-xs text-ink-light mt-2">${t('characterNotVisited')}</p>` : ''}
  `;
  wrapper.appendChild(intro);

  // 活跃任务（有新信函的）
  if (active.length > 0) {
    wrapper.appendChild(renderTaskSection(t('questActiveTitle'), active, 'active', planeId, charId, char));
  }

  // 待回信提交
  if (pending.length > 0) {
    wrapper.appendChild(renderTaskSection(t('questPendingTitle'), pending, 'pending', planeId, charId, char));
  }

  // 已完成
  if (completed.length > 0) {
    wrapper.appendChild(renderTaskSection(t('completed'), completed, 'completed', planeId, charId, char));
  }
  ...
  if (active.length === 0 && pending.length === 0 && completed.length === 0) {
    ...
    empty.innerHTML = `
      ...
      <p class="text-ink-light">${t('questEmptyHint')}</p>
    `;
...
function renderTaskSection(title, tasks, status, planeId, charId, char) {
  ...
  tasks.forEach(task => {
    const typeLabel = t(`questType_${task.type}`) || task.type;
    ...
    if (status === 'active') {
        ...
        <div class="bg-white rounded-xl p-4 border-2 border-magic-gold/30">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div class="text-xs text-ink-light mt-1">${t('questRewardLabel').replace('{reward}', formatReward(task.reward))}</div>
            </div>
      ...
      html += `
        <div class="bg-green-50 rounded-xl p-4 border-2 border-green-300">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="text-xs text-green-600 font-bold mb-1">${typeLabel} · ${t('completed')}</div>
              <div class="font-bold text-sm">${task.summary}</div>
              <div class="text-xs text-ink-light mt-1">${t('questRewardLabel').replace('{reward}', formatReward(task.reward))}</div>
            </div>
  ...
  if (mode === 'offer') {
    ...
    content.innerHTML = `
      <div class="text-center mb-4">
        <span class="text-4xl">${char.emoji}</span>
        <h3 class="font-display text-lg font-bold mt-2">${t('letterFromName').replace('{name}', char.name)}</h3>
      </div>
      ...
      <div class="bg-amber-50 rounded-lg p-3 mb-4">
        <div class="text-xs text-ink-light mb-1">${t('commissionContent')}</div>
        <div class="font-bold text-sm">${task.summary}</div>
        <div class="text-xs text-magic-gold mt-1">${t('rewardLabel').replace('{reward}', formatReward(task.reward))}</div>
      </div>
      <div class="text-center text-xs text-ink-light mb-4">
        ${t('questAutoAccepted')}
      </div>
      <div class="text-center">
        <button class="close-letter-btn px-6 py-2.5 bg-magic-gold text-white rounded-lg font-bold hover:shadow-lg transition-all">${t('gotIt')}</button>
      </div>
    `;
  } else {
    // 回信提交 — 确认提交
    ...
    content.innerHTML = `
      <div class="text-center mb-4">
        <span class="text-4xl">${char.emoji}</span>
        <h3 class="font-display text-lg font-bold mt-2">${t('replyToName').replace('{name}', char.name)}</h3>
      </div>
      <div class="bg-white/60 rounded-xl p-4 mb-4 italic text-sm text-ink leading-relaxed">
        <p class="mb-3">${letter.body}</p>
        <p class="text-right">${letter.closing}</p>
      ...
      <div class="bg-green-50 rounded-lg p-3 mb-4">
        <div class="text-xs text-green-600 mb-1">✅ ${t('questCompleted')}</div>
        <div class="font-bold text-sm">${task.summary}</div>
        <div class="text-xs text-magic-gold mt-1">${t('rewardLabel').replace('{reward}', formatReward(task.reward))}</div>
      </div>
      <div class="flex justify-center gap-3">
        <button class="cancel-submit-btn px-5 py-2.5 bg-wood/20 text-ink-light rounded-lg font-bold hover:bg-wood/30 transition-all">${t('later')}</button>
        <button class="confirm-submit-btn px-5 py-2.5 bg-green-600 text-white rounded-lg font-bold hover:shadow-lg transition-all">📮 ${t('sendReply')}</button>
      </div>
    `;
  }
...
function formatReward(reward) {
  const parts = [];
  if (reward.coins) parts.push(t('rewardCoins').replace('{n}', reward.coins));
  if (reward.atmo) parts.push(t('rewardAtmosphere').replace('{n}', reward.atmo));
  if (reward.memento) parts.push('🏛️ ' + t('memento'));
  if (reward.letter) parts.push('📜 ' + t('replyLetter'));
