...
js/                     ← 逻辑层
  app.js                ← 入口：初始化/页面切换/全局编排/新手引导/成就检测接入
  state.js              ← 全局状态 + localStorage 序列化 + 旧档迁移
  timer.js              ← 计时器（番茄钟25min/倒计时45min/正计时）
  storage.js            ← 智慧之光/氛围/历史/连击 原子读写
  visitors.js           ← 访客逻辑（纯逻辑，不碰DOM）
  shop.js               ← 商店逻辑：借阅区/缮写室升级 + 书籍刷新/购买
  books.js              ← 书籍解锁/进度计算
  achievements.js       ← 成就引擎：30 成就定义 + 条件判定 + toast
...
| Key | 内容 | 管理方 |
|-----|------|--------|
| `library_state` | 核心存档（books/library/visitors/history） | state.js |
